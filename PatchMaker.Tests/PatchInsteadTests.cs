...
namespace PatchMaker.Tests
{
    ...
    public class PatchInsteadTests
    {
        ...
        [TestMethod]
        public void PatchInstead_Constructor_WithRBC_Works()
        {
            var newElement = new XElement("test");

            var sut = new PatchInstead("/sites", "*[]", newElement, new ConfigRule[] { new ConfigRule("role", "require", "Standalone") });
        ...
        [TestMethod]
        public void InsertElement_PatchGenerator_Accepts_Insert_WithRBC()
        {
            var ins = new PatchInstead("/sitecore/sites", "site[@name='a']", new XElement("site", new XAttribute("name", "c")), new ConfigRule[] { new ConfigRule("https://x/role/", "require", "Standalone") });
            var xml = XDocument.Parse("<sitecore><sites><site name=\"a\"/></sites></sitecore>");
